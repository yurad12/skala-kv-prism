...
발표에서 차별점으로 설명할 부분입니다. 무엇을 정했고 왜 그렇게 정했는지입니다.

**1. 기술은 사람이 골랐습니다**
에이전트가 고르면 실행할 때마다 대상이 바뀔 수 있고, 고른 뒤에 논문을 인덱싱해야 해서 시간이 늘어납니다. 자료가 네 관점에 다 있는지 보는 일은 사람이 하는 편이 낫다고 판단했습니다.

**2. RAG 문서는 논문 2편으로만 한정했습니다**
벤더 백서나 뉴스는 웹 검색으로 가져와 출처와 함께 인용하고 RAG 풀에는 넣지 않았습니다. 논문 원문에서 수치와 실험 조건을 뽑는 일과, 시장 반응을 찾는 일을 도구 단위로 분리했습니다.

**3. 검색 방식은 감이 아니라 측정으로 정했습니다**
사람이 만든 질문 24개에 정답 청크를 붙여 파서, 임베딩 모델, 검색 방식을 각각 쟀습니다. 한국어 질의는 키워드 검색(BM25)에서 정답을 거의 못 찾아서 질의를 영어로 고정했습니다. 표는 아래 RAG 절에 있습니다.

**4. State를 먼저 확정하고 4명이 병렬로 만들었습니다**
공유 데이터 구조(키 11개)를 첫날 합의하고, 각자 맡은 노드를 가짜 데이터로 검증하며 만들었습니다. 다른 사람 코드를 기다린 시간이 거의 없었습니다.

**5. 규칙은 프롬프트가 아니라 코드로 막았습니다**
추천 · 우열 표현, TRL 단계 상한, 인용 형식은 프롬프트에만 적으면 지켜지지 않았습니다. 저장 전에 코드가 검사하고, 어긋나면 낮추거나 다시 요청합니다.

**6. 만드는 모델과 검사하는 모델을 나눴습니다**
같은 모델이 자기 결과를 검사하면 통과시키는 쪽으로 기울어서, 검증(Judge)은 별도 모델로 두었습니다.
...
| 도메인 평가 (domain) | 데이터센터 서빙 시나리오에서 처리량 · 비용, 정확도 위험, 인프라 조건을 평가합니다 | X | `domain_eval`, `sources` | `web_search` |
...
설계 문서에서는 도메인 평가도 RAG를 쓰기로 했으나, 현재 코드는 웹 검색만 씁니다. 기술 조사 결과(`research`)를 입력으로 받아 논문 내용과 대조합니다.
...
RAG는 기술 조사(research)가 씁니다. 문서 풀은 논문 2편(38쪽), 검색기는 `rag_retrieve(doc_id, query, k)` 하나입니다.
...
2026-09-22 실행 기준입니다. 개발용 모델(gpt-5.6-luna)로 돌렸습니다.
...
1. KV 용량 절감과 추론 품질 · 역양자화 비용의 상충 (TurboQuant)
2. 원격 메모리 용량 확장과 HBM 지연 · 대역폭의 상충 (ITME)
3. 소프트웨어 압축이 메모리 수요를 줄인다는 관점과, 고성능 메모리 · 인터커넥트 수요가 이동한다는 관점의 상충
...
**설계와 다른 부분** — 도메인 평가가 RAG를 쓰지 않고, `fetch_and_summarize` 도구는 호출되지 않으며, Judge의 인용 검사가 코드 대조 없이 LLM 판정만으로 이뤄집니다.

## Lessons Learned

**규칙은 코드에 둬야 지켜집니다.** 중립성과 TRL 상한을 프롬프트에만 적었을 때는 어겼습니다. 저장 전 검사로 옮긴 뒤에야 지켜졌습니다. 반대로 단어 포함 같은 느슨한 검사는 모델이 우회합니다.

**모델이 약하면 형식을 지키지 않습니다.** 같은 코드를 gpt-4o-mini로 돌리면 인용을 `[소스_id: tq-paper]`나 기관명으로 써서 검사에 걸렸고, gpt-4o부터는 한 번도 틀리지 않았습니다. 그래도 12자리 난수 ID는 강한 모델도 한 글자를 흘려서, "무엇이 틀렸는지 알려주고 한 번 다시 요청"하는 단계를 넣었습니다.

**구조화 출력의 줄바꿈은 믿을 수 없습니다.** 두 줄로 요구한 근거를 모델이 한 줄로 이어 썼습니다. 줄 단위로 읽던 코드를 라벨 위치로 읽게 바꿨습니다.

**한국어 질의는 영어 논문 검색에서 불리합니다.** 키워드 검색에서 정답을 거의 못 찾았고(Hit@5 0.458), 의미 검색과 합쳐도 나아지지 않았습니다. 측정으로 확인하고 영어로 고정했습니다.

**State를 먼저 정하는 것이 병렬 작업의 조건이었습니다.** 키와 하위 구조를 먼저 합의해서 각자 가짜 데이터로 자기 노드를 검증할 수 있었습니다.
...
- 이진욱 : RAG 파이프라인 · 기술 조사 (PDF 로딩 · 청킹 · 인덱싱, `rag_retrieve`, `research` 노드, 골든 근거 세트와 파서 · 임베딩 · 검색 방식 실측). 발표 Q&A는 RAG · 문서 처리 담당
- 이민경 : 웹 도구 · 관점 평가 (`web_search`, 검색 결과 캐싱, `market` · `stakeholder` · `domain` 노드와 프롬프트). 발표 Q&A는 웹 수집 · 에이전트 동작 담당
- 정유정 : 오케스트레이션 · 검증 (공용 State 스키마, LangGraph 병렬 실행과 재실행 루프, `judge` 노드, `app.py`). 10분 발표 진행
- 오승민 : 평가 종합 · 보고서 (`synthesize` 노드의 TRL 추정과 상충 지점, `report` 노드의 장별 생성과 REFERENCE, 금지 표현 검사, PDF 변환). 발표 Q&A는 매트릭스 · 종합 결론 담당
